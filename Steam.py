    ...
    def calc(self):
        ...
        if self.T is not None:
            if self.T>Tsat: #interpolate with griddata
                ...
                self.h = float(griddata((tcol, pcol), hcol, (self.T, self.p))) #$JES MISSING CODE HERE$  #use griddata to interpolate with T & P the superheated table
                self.s = float(griddata((tcol, pcol), scol, (self.T, self.p))) #$JES MISSING CODE HERE$  #use griddata to interpolate with T & P the superheated table
                self.x=1.0
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region='Superheated'
                self.T = float(griddata((hcol, pcol), tcol, (self.h, self.p))) #$JES MISSING CODE HERE$  #use griddata to interpolate with h & P the superheated table
                self.s = float(griddata((hcol, pcol), scol, (self.h, self.p))) #$JES MISSING CODE HERE$  #use griddata to interpolate with h & P the superheated table
        elif self.s!=None:
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region = 'Superheated'
                self.T = float(griddata((scol, pcol), tcol, (self.s, self.p))) #$JES MISSING CODE HERE$ #use griddata to interpolate with s & P the superheated table
                self.h = float(griddata((scol, pcol), hcol, (self.s, self.p))) #$JES MISSING CODE HERE$  #use griddata to interpolate with s & P the superheated table
        #endregion
