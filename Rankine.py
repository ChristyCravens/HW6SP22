...
class rankine():
    ...
    def calc_efficiency(self):
        ...
        if(self.t_high==None):
            ...
            #JES MISSING CODE$ # instantiate a steam object with conditions of state 1 as saturated steam, named 'Turbine Inlet'
        else:
            self.state1= steam(self.p_high, T=self.t_high, name='Turbine Inlet')
            #JES MISSING CODE$ # instantiate a steam object with conditions of state 1 at t_high, named 'Turbine Inlet'
        #state 2: turbine exit (p_low, s=s_turbine inlet) two-phase
        self.state2= steam(self.p_low, s=self.state1.s, name='Turbine Exit')
        #JES MISSING CODE$ # instantiate a steam object with conditions of state 2, named 'Turbine Exit'
        #state 3: pump inlet (p_low, x=0) saturated liquid
        self.state3= steam(self.p_low, x=0, name='Pump Inlet')
        #JES MISSING CODE$ # instantiate a steam object with conditions of state 3 as saturated liquid, named 'Pump Inlet'
        ...
        self.turbine_work= self.state1.h - self.state2.h #$JES MISSING CODE$ # calculate turbine work
        self.pump_work= self.state4.h - self.state3.h #$JES MISSING CODE$ # calculate pump work
        self.heat_added= self.state1.h - self.state4.h #$JES MISSING CODE$ # calculate heat added
...
def main():
    rankine1= rankine(8, 8000, t_high=500, name='Rankine Cycle - Superheated at Turbine Inlet') #$JES MISSING CODE$ #instantiate a rankine object to test it.
    #t_high is specified
